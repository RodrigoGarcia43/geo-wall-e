...
namespace GeoWall_E
{
    class SequenceInputPoint : Input
    {
        public SequenceInputPoint(CodeLocation location) : base(location)
        {
        }
        ...
        {
            List<Point> item = new List<Point>();
            for (int i = 0; i < 5; i++)
            {
                body = new Point();
                item.Add((Point)body);
            ...
            table.Executing.Add(Id, new VarInfo(item, ExpressionType.PointSequence));
        }
    }
}
